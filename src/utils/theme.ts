...
export const ARTIST_THEME_MAP: Record<string, ThemeOption> = {
    teeteepor: { bg: 'bg-emerald-50', text: 'text-emerald-600', border: 'border-emerald-100' },
    teetee: { bg: 'bg-purple-50', text: 'text-purple-600', border: 'border-purple-100' },
...
export const TYPE_THEME_MAP: Record<string, ThemeOption> = {
    ...
    'Performance': { bg: 'bg-sky-50', text: 'text-sky-600', border: 'border-sky-100' },
    'Cover': { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-100' },
    'Special': { bg: 'bg-violet-50', text: 'text-violet-600', border: 'border-violet-100' },
};

// --- Helper Functions ---
export const getArtistTheme = (artist: string): ThemeOption => {
    const key = artist?.toLowerCase().replace(/\s/g, '') || '';
    return ARTIST_THEME_MAP[key] || { bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-slate-100' };
};
...
export const getTypeTheme = (type: string): ThemeOption => {
    return TYPE_THEME_MAP[type] || { bg: 'bg-slate-50', text: 'text-slate-600', border: 'border-slate-100' };
};